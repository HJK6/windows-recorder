 ...
 * real `save-recording` IPC, and the real recording-store. Logs what the WebM
...
app.whenReady().then(() => {
  ipcMain.handle('save-recording', async (_e, bytes, sessionId) => {
    ...
    try {
      const res = await recordingStore.saveRecording(DOCS, bytes, sessionId);
